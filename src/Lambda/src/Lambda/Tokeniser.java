...
public class Tokeniser {
    ...
    public static String[] Tokenise(String input){
        ...
        if(input.startsWith(Character.toString(Constants.INCLUDE_FILE))) {
            tokens.add(Character.toString(Constants.INCLUDE_FILE));
            String filenames = input.substring(1, input.length()).trim();
            for(String filename: filenames.split(Character.toString(Constants.SPACE))) {
                tokens.add(filename);
            }            
        } else {
            String token = "";
